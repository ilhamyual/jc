...
if( empty( $_SESSION['id_user'] ) ){
	...
	if( isset( $_REQUEST['aksi'] )){
		...
		echo '
			...
			<div style="background-image:url(image/1.jpg)" class="container">
				<h3 style="margin-bottom: -20px;"><b>Daftar Transaksi</b></h3>
			...
			//skrip untuk menampilkan data dari database
		 	...
		 	if(mysqli_num_rows($sql) > 0){
	 			...
	 			echo '
				   ...
				   <tr>
					 <td><b>'.$no.'</b></td>
					 <td><b>'.$row['no_nota'].'</b></td>
					 <td><b>'.$row['nama'].'</b></td>
					 <td><b>'.$row['jenis'].'</b></td>
					 <td><b>RP. '.number_format($row['total']).'</b></td>
					 <td><b>'.date("d M Y", strtotime($row['tanggal'])).'</b></td>
					 <td>
